	...
	$.fn.Validform.sn={
		...
		regcheck:function(type,gets){
			...
			switch(type){
				case "*":
					return true;
				case "datetime": //2000-01-01 12:01:01
					reg= /^\d{4}-\d{2}-\d{2} \d{1,2}:\d{1,2}:\d{1,2}$/;
				...
				case "p":
					...
					return reg.test(gets);
				case "m": //手机
					reg= /^13[0-9]{1}[0-9]{8}$|15[0-9]{1}[0-9]{8}$|18[0-9]{1}[0-9]{8}$|17[0-9]{1}[0-9]{8}$/;
					return reg.test(gets);
				case "e": //电子邮件
					reg = /\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/;
					return reg.test(gets);
